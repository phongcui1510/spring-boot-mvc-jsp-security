...
//	                        var selected = getSelectedItems();
	        				...
	        				$.ajax({
	                            ...
	                            success: function (response) {
	                            	var item = $('<li id='+response+'><span><span class="glyphicon glyphicon-folder-open"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
            ...
            if (options.editable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .edit > button').attr('title', options.i18n.editTip).click(function () {
                    ...
                    else if ($(selected).hasClass('parent_li')) {
...
//                                $(selected).parent('ul').remove();
            ...
            //viewable
            if (options.viewable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="view"><button class="btn btn-default btn-sm btn-warning disabled"><span class="glyphicon glyphicon-eye-open"></span></button></div> ');
                $(easyTree).find('.easy-tree-toolbar .view > button').click(function () {
                    var selected = getSelectedItems();
                    if (selected.length <= 0) {
//                        $(easyTree).prepend(warningAlert);
//                        $(easyTree).find('.alert .alert-content').html(options.i18n.deleteNull);
                    	...
                    	window.location.href = '/ProofMgmSystem/proof/view?id='+$(selected).attr('id');
                    }
                });
            }
            ...
            $(easyTree).delegate('span.glyphicon-file', 'click', function (e) {
            	var parent = $(this).parent().parent();
            	window.open('/ProofMgmSystem/proof/view?id='+$(parent).attr('id'));
            });
            ...
            if (options.selectable) {
                ...
                $(easyTree).find('li > span > a').click(function (e) {
                    ...
                    if (options.deletable || options.editable || options.addable || options.viewable) {
                        ...
                        if (options.viewable) {
                            if (selected.length <= 0 || selected.length > 1 || $(selected).hasClass('parent_li'))
                                $(easyTree).find('.easy-tree-toolbar .view > button').addClass('disabled');
                            else
                                $(easyTree).find('.easy-tree-toolbar .view > button').removeClass('disabled');
                        }
                    }
            ...
            //modified
            if (options.role == 'USER3') {
            	$('.easy-tree-toolbar .create > button').unbind();
            	$('.easy-tree-toolbar .edit > button').unbind();
            	$('.easy-tree-toolbar .remove > button').unbind();
            	$('.easy-tree-toolbar .create > button').click(function(){
            		var selected = getSelectedItems();
            		if ($(selected).attr('id') != undefined && $(selected).hasClass('parent_li')) {
            			window.location.href = '/ProofMgmSystem/proof/create?type=LEAF&parentId='+$(selected).attr('id');
            		}
            	});
            	$('.easy-tree-toolbar .edit > button').click(function(){
            		...
            		if ($(selected).attr('id') != undefined) {
            			if ($(selected).hasClass('parent_li')) {
            				alert("Bạn không thể chỉnh sửa!");
            				$(this).addClass('disabled');
            				return;
            			}
            			window.location.href = '/ProofMgmSystem/proof/edit?id='+$(selected).attr('id');
            		}
            	});
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    var selected = getSelectedItems();
...
//                                    $(selected).parent('ul').remove();
            ...
            } else if (options.role != 'USER1' && options.role != 'USER3' && options.role != 'USER4') {
            	$('.easy-tree-toolbar .create > button').unbind();
            	$('.easy-tree-toolbar .edit > button').unbind();
            	$('.easy-tree-toolbar .remove > button').unbind();
            }
         ...
         // Get selected items
            ...
            $('li.parent_li').each(function(index, value){
            	if ($(value).find('ul > li').length > 0) {
//            		alert($(value).attr("id"));
            		$(this).find('a').first().css('color','#ff8080');
            	}
            });
        });
    };
